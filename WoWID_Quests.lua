...
local QTR_Reklama = {
      ...
      TEXT1 = "WoWID-Quests - addon yang menampilkan quest terjemahan (warcraftid.id)",
      TEXT2 = "WoWID-Quests - addon yang menampilkan quest dalam bahasa Indonesia (warcraftid.id)",};
...
local p_race = {
      ...
      ["Manusia"] = { M1="Manusia", D1="Manusia", C1="Ke Manusia", B1="Manusia", N1="sebagai Manusia", K1="in Manusia", W1="Manusia" },
...
function QTR_BlizzardOptions()
	...
	local QTROptions = CreateFrame("FRAME", "WoWID_Quests_Options", SettingsPanel);
	QTROptions:SetScript("OnHide", nil);
	QTROptions.name = "WoWID";
	QTROptions.refresh = function (self) QTR_SetCheckButtonState() end;
	--InterfaceOptions_AddCategory(QTROptions);
	local category = Settings.RegisterCanvasLayoutCategory(QTROptions, "WoWID");
  ...
  QTROptionsHeader:SetText("WoWID-Quests, ver. "..QTR_version.." ("..QTR_base..") by Sarimin © 2025");
  ...
  QTRDateOfBase:SetText("Tanggal Basis Terjemahan: "..QTR_date);
  QTRDateOfBase:SetFont(QTR_Font2, 16);

  local QTRCheckButton0 = CreateFrame("CheckButton", "QTRCheckButton0", QTROptions, "UICheckButtonTemplate");
  ...
  local QTRCheckButton3 = CreateFrame("CheckButton", "QTRCheckButton3", QTROptions, "UICheckButtonTemplate");
  ...
  QTROptionsMode2:SetText("Nama pemain dalam berbagai kasus:");
  ...
  QTRName7:SetText("Vokatif:");
  QTRName7:SetFont(QTR_Font2, 13);

  local QTRCheckButton5 = CreateFrame("CheckButton", "QTRCheckButton5", QTROptions, "UICheckButtonTemplate");
...
--[[  local QTR_slider = CreateFrame("Slider","MyAddonSlider",QTROptions,'OptionsSliderTemplate');
  ...
  local QTRCheckText1 = CreateFrame("CheckButton", "QTRCheckText1", QTROptions, "UICheckButtonTemplate");
  ...
  local QTRCheckText2 = CreateFrame("CheckButton", "QTRCheckText2", QTROptions, "UICheckButtonTemplate");
  ...
  local QTRCheckOther1 = CreateFrame("CheckButton", "QTRCheckOther1", QTROptions, "UICheckButtonTemplate");
  ...
  local QTRCheckOther2 = CreateFrame("CheckButton", "QTRCheckOther2", QTROptions, "UICheckButtonTemplate");
  ...
  local QTRCheckOther3 = CreateFrame("CheckButton", "QTRCheckOther3", QTROptions, "UICheckButtonTemplate");
...
--   QTR_ToggleButton0:Hide();        -- Kami menutup tombol QTR selama percakapan dengan NPC (deskripsi dan pemilihan tugas)
   ...
   if (Nazwa_NPC and GreetingText) then
      ...
      if (string.find(Greeting_Text," ")==nil) then         -- Ini bukan teks dalam bahasa indoneisa (tidak ada ruang yang sulit)
...
--         QTR_quest_EN.objectives = QuestLogObjectivesText:GetText();
         ...
         if (isImmersion()) then
            if (q_ID==0) then
               if (ImmersionFrame.TitleButtons:IsVisible()) then
                  QTR_ToggleButton4:SetText("Pilih pencarian terlebih dahulu");
               end
            else
      ...
      end -- Sebuah pencarian di pangkalan diterjemahkan
   else	--terjemahan off
      QTR_ToggleButton0:Disable();
      QTR_ToggleButton1:Disable();
      QTR_ToggleButton2:Disable();
...
--            QTR_ToggleButton4:Disable();
--         end
      if ( QTR_QuestData[str_ID] ) then	--Tapi ada terjemahan dalam database
...
function QTR_PrepareDelay(czas)     -- Dipanggil setelah mengklik nama pencarian dari daftar NPC
   if (czas==1) then
...
function QTR_Immersion()   -- Memanggil terjemahan dengan penundaan 0,2 detik
...
function QTR_Immersion_OFF()   
  ...
  if (strlen(QTR_quest_EN.details)>0) then                                    -- Kembalikan teks asli
...
-- masih menangani NPC_GENDER(x;y)
   local nr_1, nr_2, nr_3 = 0;
   local QTR_forma = "";
   local nr_poz = string.find(msg, "NPC_GENDER");    -- Ketika dia tidak menemukannya, itu: nil
   while (nr_poz and nr_poz>0) do
      nr_1 = nr_poz + 1;   
      while (string.sub(msg, nr_1, nr_1) ~= "(") do
         nr_1 = nr_1 + 1;
...
-- masih menangani OWN_NAME(EN;ID)
   local nr_1, nr_2, nr_3 = 0;
   local QTR_forma = "";
   local nr_poz = string.find(msg, "OWN_NAME");    -- Ketika dia tidak menemukannya, itu: nil
   while (nr_poz and nr_poz>0) do
      nr_1 = nr_poz + 1;   
      while (string.sub(msg, nr_1, nr_1) ~= "(") do
         nr_1 = nr_1 + 1;
